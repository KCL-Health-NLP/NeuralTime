...
def convert_examples_to_features(examples, max_seq_length, tokenizer, out_path):
    ...
    for  idx, example in enumerate(examples):
        ...
        features_list.append([input_ids, input_mask, segment_ids, label_id, example.label])

    print('Max Sequence Length: %d' % max_len)


    features_df = pd.DataFrame(features_list, columns=['input_id', 'input_mask', 'segment_id', 'label_id', 'label'])
    print('FEATURES ')
    print(features_df)
...
def clinical_bert(train_features, test_set_features, do_train, learning_rate, num_train_epochs, train_batch_size, val_prop = 0.1, undersample = 0.5, multilabel_mode = False, fp16 = True):
    ...
    if do_train:
        ...
        if undersample:
            undersample_mask = [(random.random()>1 - undersample) if label == 0 else True for label in train_set_features['label_id']]
            train_set_features = train_set_features[undersample_mask]
        ...
        if val_prop > 0:
            ...
            validation_dataloader = DataLoader(val_data, sampler=val_sampler, batch_size=train_batch_size)
        ...
        for _ in trange(int(num_train_epochs), desc="Epoch"):
            ...
            if val_prop > 0:
                run_eval(validation_dataloader)
        if val_prop > 0:
            pd.DataFrame(val_metrics).to_excel('training_scores_' + str(datetime.now()) + '.xlsx')
        ...
        run_eval(test_dataloader)

    now = str(datetime.now())
    os.mkdir('bert_weights_' + now)
    bert_model.save_pretrained('bert_weights_' + now)
    return bert_model
