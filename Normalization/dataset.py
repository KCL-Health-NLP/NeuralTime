...
class TupleInstance():
    ...
    of a training/test example, while InputExample contains input text and is adapted to Bert format.
    """

    def __init__(self, docname, rtimexe, ptimexe, is_anchor, before, after, equal, test):
...
class AnnotatedDataset():
    ...
    def __init__(self, inference = False):
        ...
        if not inference:
            ...
            self.tuple_df = pd.read_excel((path / 'inference_tuple_df.xlsx').as_posix())
            self.train_inputs = pd.read_excel((path / 'inference_train_inputs.xlsx').as_posix())
    ...
    def convert_to_tuples(self, out_path = None, inference = False):
        ...
        for timexe in r_timexes.to_dict('records'):
            ...
            for pa in potential_anchors:
                ...
                tuple_instances += [TupleInstance(docname, id1, pa['id'], is_anchor, relation == 'BEFORE', relation == 'EQUAL', relation == 'AFTER', test )]
    ...
    def generate_inputs(self, type = 'train', out_path = None):
        ...
        if type == 'test':
            tuple_data = self.tuple_df[self.tuple_df['test']]
        else:
            tuple_data = self.tuple_df[self.tuple_df['test'] == False]
    ...
    def get_examples(self, type = 'train'):
        ...
        if type == 'train':
            ...
            examples = [InputExample(id_, text_a, text_b, label) for id_, text_a, text_b, label in zip(self.train_inputs.id, self.train_inputs.text_a, self.train_inputs.text_b, self.train_inputs.label)]
        elif type == 'test':
            examples = [InputExample(id_, text_a, text_b, label) for id_, text_a, text_b, label in zip(self.test_inputs.id, self.test_inputs.text_a, self.test_inputs.text_b,
